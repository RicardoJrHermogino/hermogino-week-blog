export const week12 = {
  id: 12,
  title: "Assisted Patient Data Encoding & Final System Turnover Discussion",
  category: "12th Week",
  image: "/picture/week_12/7.jpg",
  date: "February 14, 2025",
  description: "In our final week at Gubat RHU, after completing our system development, we returned to assist with encoding patient data into the IClinicSys system. We also discussed with the staff how to maintain the system, potential hosting options, and its long-term use. At the end of the week, we were awarded certificates and had a photo with Dr. Ronald E. Ferreras, Municipal Health Officer of Gubat, and Sir Jay-Ar Jarabejo, IT staff and our training supervisor.",
  content: `
    <h2>System Completion and Final Week Assistance</h2>
    <p>In our final week at Gubat RHU, we shifted our focus back to assisting the staff in encoding patient data into the IClinicSys system, ensuring records were accurate and updated before we concluded our OJT.</p>
    
    <h3>System Turnover and Maintenance Discussion</h3>
    <p>We held a meeting with Dr. Ronald E. Ferreras and Sir Jay-Ar Jarabejo to discuss how the system we developed could be maintained moving forward. Topics included managing user access, updating records, and the possibility of hosting the system under a dedicated domain for better accessibility and scalability.</p>
    
    <h3>Certificate Awarding</h3>
    <p>At the end of the week, we were honored with certificates of completion and had a commemorative photo with Dr. Ronald E. Ferreras, the Municipal Health Officer of Gubat, and Sir Jay-Ar Jarabejo, the IT staff and our training supervisor.</p>
    
    <p>This final week allowed us to wrap up our technical contributions while ensuring that the RHU staff felt confident in continuing to use and maintain the system we developed.</p>
  `,
  gallery: [
    {
      type: "image",
      source: "/picture/week_12/1.jpg",
      caption: "Assisting with patient data encoding"
    },
    {
      type: "image",
      source: "/picture/week_12/2.jpg",
      caption: "Discussing system maintenance and hosting options"
    },
    {
      type: "image",
      source: "/picture/week_12/3.jpg",
      caption: "Final system review with RHU staff"
    },
    {
      type: "image",
      source: "/picture/week_12/5.jpg",
      caption: "Receiving certificate of completion"
    },
    {
      type: "image",
      source: "/picture/week_12/7.jpg",
      caption: "Commemorative photo with Dr. Ronald E. Ferreras and Sir Jay-Ar Jarabejo"
    },
  ]
};
